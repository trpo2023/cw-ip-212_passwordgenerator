...
// функция, генерирующая случайное целое число в заданном диапазоне
int random_int(int min, int max) {
    return rand() % (max - min + 1) + min;
}

// функция, генерирующая случайный символ из заданного набора
char random_symbol(char *symbols, int num_symbols) {
    int index = rand() % num_symbols;
    return symbols[index];
}

// функция, генерирующая пароль заданной длины с заданным набором символов
